...
public class AnimatorPlayCall : MonoBehaviour
{
	...
	class AnimatorPlayCallParam
	{
		...
		public bool Check()
		{
			...
			if (CurInfo.shortNameHash == shortNameHash && shortNameHash != NextInfo.shortNameHash && CurInfo.normalizedTime > 0.99f)
			{
				Action.Invoke();
				return true;
			}
...
public static class AnimatorExtension
{
	...
	public static void Play(this Animator animater, string name, int layer, System.Action action)
	{
		animater.Play(name, layer);
		AnimatorPlayCall.I.OnIsPlayOver(animater, true, action);
	}
