...
namespace LitJson
{
    ...
    internal struct ObjectMetadata
    {
        ...
        private bool is_list;
        private Type key_type;//如果是字典，那么它的key的类型
    ...
    internal struct InheritMetadata
    {
        private Type type;
    ...
    public class JsonMapper
    {
        ...
        private static IDictionary<Type, InheritMetadata> inherit_metadata;
        private static readonly object inherit_metadata_lock = new object();
        ...
        static JsonMapper()
        {
            ...
            inherit_metadata = new Dictionary<Type, InheritMetadata>();
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            if (type.GetInterface("System.Collections.IDictionary") != null)
            {
                ...
                if (m == null)
                {
                    Debug.LogError("找不到索引器 不能确定key的类型:" + type.Name);
                ...
                else
                {
                    ...
                    data.KeyType = m.GetParameters()[0].ParameterType;
                    data.ElementType = m.ReturnType;
                }
            ...
            else if (type.GetInterface("System.Collections.IList") != null)
            {
                ...
                MethodInfo m = type.GetMethod("get_Item");//list的索引器的返回值就是对应类型
                if (m == null)
                    Debug.LogError("找不到索引器 不能确定值的类型:" + type.Name);
                else
                    data.ElementType = m.ReturnType;
            }
                ...
                reader.Token == JsonToken.Boolean)
            {
                ...
                switch (reader.Token)
                {
                    case JsonToken.Int:
                        ...
                        if (value_type == typeof(PropertyValue))
                            return new PropertyValue((int)reader.Value);
                        break;
                    case JsonToken.Long:
                        if (value_type == typeof(int))
                            return (int)(long)reader.Value;
                        if (value_type == typeof(PropertyValue))
                            return new PropertyValue((long)reader.Value);
                        break;
                    case JsonToken.String:
                        if (value_type == typeof(PropertyValue))
                            return new PropertyValue((string)reader.Value);
                        break;
                    case JsonToken.Double:
                        ...
                        if (value_type == typeof(PropertyValue))
                            return new PropertyValue((float)(double)reader.Value);
                        break;
                }
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                if (i_data.IsInherit)
                {
                    ...
                    if (property == "__type")
                    {
                        ...
                        if (trueType != null)
                        {
                            ...
                            Debug.LogError("找不到类名对应的类型:{0}"+ trueTypeName);
                    }
                    else
                        Debug.LogError("逻辑错误，可能是老版本");
                }
                ...
                while (true)
                {
                    ...
                    else if (t_data.IsList)
                    {
                        ...
                        {
                            Debug.LogError("json尝试将键值对转为数组失败，类型：{0}，转换失败的key:{1}" + value_type.Name + property);
                            ReadSkip(reader);
                            continue;
                        }
                    ...
                    else if(t_data.IsDictionary)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Debug.LogError("json尝试将键值对转换成:{0}时失败，转换失败的key:{1}" + value_type.Name + property);
                                ReadSkip(reader);
                                continue;
                            }
                                        ...
                                        int depth)
        {
            ...
            {
                ((PropertyValue)obj).ToJson(writer);
                return;
            }
