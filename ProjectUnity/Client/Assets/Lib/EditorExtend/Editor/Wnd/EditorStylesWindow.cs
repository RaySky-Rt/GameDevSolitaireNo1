...
public class EditorStylesWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        using (AutoBeginScrollView a = new AutoBeginScrollView(pos))
        {
            ...
            EditorGUILayoutEx editorGUILayoutEx = EditorGUILayoutEx.instance;

            EditorGUILayoutEx.FadeArea fadeArea = editorGUILayoutEx.BeginFadeArea(bOpenArea, "区域块", "area", EditorStyleEx.PixelBoxStyle, EditorStyleEx.TopBoxHeaderStyle);
            bOpenArea = fadeArea.open;
            if (fadeArea.Show())
            {

                EditorGUILayoutEx.FadeArea topFadeArea = editorGUILayoutEx.BeginFadeArea(bOpenArea3, "", "areaid", EditorStyleEx.GraphBoxStyle);
                bOpenArea3 = topFadeArea.open;
                if (topFadeArea.Show())
                {
                    EditorGUILayout.HelpBox("help box", MessageType.Info);
                }
                editorGUILayoutEx.EndFadeArea();
            ...
            EditorGUILayoutEx.FadeArea fadeArea2 = editorGUILayoutEx.BeginFadeArea(bOpenArea2, "区域块", "area2", EditorStyleEx.PixelBoxStyle);
