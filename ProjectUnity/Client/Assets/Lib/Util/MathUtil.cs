...
public class MathUtil
{
    ...
    public static float Curve(EaseType easeType, float start, float end, float value)
    {
        switch (easeType)
        {
            ...
            default: Debug.LogError("未知的类型:{0}" + easeType); return end;
        }
    }
    ...
    #endregion
    #region Weight Func
    public static T GetRandomByWeight<T>(Pair<T, int>[] ary)
    {
        int len = ary.Length;
        int total = 0;
        for (int i = 0; i < len; i++)
        ...
        int random = RG.Basic.Random.Range(0, total);

        int last = 0;
        for (int i = 0; i < len; i++)
        {
            if (last <= random && random <= ary[i].v) {
