...
namespace LitJson
{
    public class PropertyValue
    {
        ...
        public int Int
        {
            get { return Check(enType.Int) ? Convert.ToInt32(_value) : 0; }
            set
            {
                if (_type == enType.Int)
                    _value = value;
                    ...
                    Debug.LogError("不能设置。属性类型是:{0} 目标类型是:{1}" + _type + enType.Int);
            }
        }
        public long Long
        {
            get { return Check(enType.Long) ? Convert.ToInt64(_value) : 0; }
            set
            {
                if (_type == enType.Long)
                    _value = value;
                else
                    Debug.LogError("不能设置。属性类型是:{0} 目标类型是:{1}" + _type + enType.Long);
            }
        }
        public float Float
        {
            get { return Check(enType.Float) ? Convert.ToSingle(_value) : 0; }
            set
            {
                if (_type == enType.Float)
                    _value = value;
                else
                    Debug.LogError("不能设置。属性类型是:{0} 目标类型是:{1}" + _type + enType.Float);
            }
        }
        public string String
        {
            get { return _value == null ? "" : Convert.ToString(_value); }
            set
            {
                if (_type == enType.String)
                    _value = value;
                else
                    Debug.LogError("不能设置。属性类型是:{0} 目标类型是:{1}" + _type + enType.String);
            }
        }
        #endregion
        ...
        #region Constructors
        public PropertyValue(PropertyValue p)
        {
            this._type = p.PropType;
            this._value = p.Value;
        }
        public PropertyValue(enType t)
        {
            _type = t;
        }

        public PropertyValue(string value, enType type)
        {
            _type = type;
            switch (_type)
            {
        ...
        bool Check(enType t)
        {
            if (t == _type ||
                ...
                return true;
            Debug.LogError("不能转换。属性类型是:{0} 目标类型是:{1}" + _type + t);
            return false;

        }
        #endregion

        public void SetValue(PropertyValue p)
        {
            if (this._type != p.PropType)
            {
                Debug.LogError("类型不同不能设置，属性类型是:{0} 目标类型是:{1}" + _type + p.PropType);
                return;
            }
            this._type = p.PropType;
            this._value = p.Value;
