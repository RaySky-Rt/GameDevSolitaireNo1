...
public class EditorUtil
{
    ...
    {
        return AssetDatabase.LoadAssetAtPath<Texture2D>(string.Format("Assets/Editor/EditorAssets/Textures/{0}.png", textureName)) as Texture2D;
    }
    ...
    {
        Debug.Log("预览图大小 {0}x{1}" + tex.width + tex.height);
    ...
    public static void SelectMaterial(Material mat)
    {
        ...
        Debug.Log("场景中找到{0}个有{1}材质的游戏对象" + os.Count + mat);
        if (os.Count > 0)
