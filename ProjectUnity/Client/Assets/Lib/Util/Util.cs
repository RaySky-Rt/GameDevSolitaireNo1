...
public static class Util
{
	...
	public static int Clamp(int value, int a, int b)
	{
		...
		//Debug.Log(string.Format("value:{0} clamp:{1}", value, a + tem));
		return a + tem;
	}
	...
	public static void SafeAbortThread(Thread t)
	{
		if (t != null && t.IsAlive)
		{
			...
			{
				Debug.LogError("error:{0} StackTrace:{1}" + e.Message + e.StackTrace);
			}
		}
	}
	...
	public static void Copy(object from, object to, BindingFlags bindingAttr, params object[] ingoreFields)
	{
		...
		{
			Debug.LogError("同一个类或者子类才能复制属性.from:{0} to:{1}" + fromType.Name + toType.Name);
			return;
		}
	...
	public static string GenerateGUID()
	{
		...
		string timePart = System.Convert.ToString(System.DateTime.Now.Ticks, 16);
	...
	{
		string path = Application.dataPath + "/Resources/" + filePath + ".json";
	...
	public static Vector2Int NormalV2(Vector2Int v)
	{
		if (v.x != 0)
		{
	...
	// 将正方形网格坐标转换为偏移坐标
	public static Vector2Int ConvertToOffsetCoordinates(int x, int y, int z)
	{
		int q = x;// - (z - (z & 1)) / 2;
		int r = z - (x - (x & 1)) / 2;
	...
	// 计算每个六边形的世界坐标
	public static Vector3 CalculateHexPosition(int x, int y, int z, float hexSize)
	{
		// 将正方形网格坐标转换为偏移坐标
		Vector2Int offsetCoordinates = ConvertToOffsetCoordinates(x, y, z);

		// 计算世界坐标
		float offsetX = hexSize * Mathf.Sqrt(3) * (offsetCoordinates.x + offsetCoordinates.y / 2f);
		float offsetY = hexSize * 3 / 2f * offsetCoordinates.y;
		return new Vector3(offsetX, 0f, offsetY);
	}
}
